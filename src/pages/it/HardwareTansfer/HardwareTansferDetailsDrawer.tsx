...
interface Props {
  open: boolean;
  item: HardwareTransferData | null;
  onClose: () => void;
}
...
export const HardwareTransferDetailsDrawer: React.FC<Props> = ({ open, item, onClose }) => {
  return (
        ...
        <div className="relative flex flex-col h-full">
          <DrawerHeader>
            <div className="flex items-start justify-between w-full">
              <DrawerTitle>{`Transfer: ${item?.id ?? ""}`}</DrawerTitle>
              <DrawerClose asChild>
          ...
          <div className="flex-1 overflow-auto p-6 pb-24">
            {item ? (
              <div className="grid grid-cols-2 gap-4 text-sm">
                {Object.entries({
                  "Transfer ID": item.id,
                  "Hardware": item.hardwareName,
                  "Serial": item.serialNumber,
                  "From": item.fromUser,
                  "To": item.toUser,
                  "Transfer Date": formatDate(item.transferDate),
                  "Expected Return": formatDate(item.expectedReturnDate),
                  "Type": item.transferType,
                  "Condition": item.hardwareCondition,
