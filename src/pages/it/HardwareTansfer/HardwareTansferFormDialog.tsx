...
export const HardwareTransferFormDialog: React.FC<Props> = ({ open, mode, form, onChange, onSubmit, onClose, onDelete, submitting = false }) => {
  return (
    <Dialog open={open} onOpenChange={(v) => (!v ? onClose() : null)}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{mode === "add" ? "New Transfer" : `Edit: ${form.id ?? ""}`}</DialogTitle>
          <DialogDescription>{mode === "add" ? "Create a new hardware transfer." : "Update transfer details."}</DialogDescription>
        </DialogHeader>
