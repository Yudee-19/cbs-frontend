...
interface Props {
  open: boolean;
  item: NetworkEquipmentData | null;
  onClose: () => void;
}
...
const EquipmentDetailsDrawer: React.FC<Props> = ({ open, item, onClose }) => {
  return (
        ...
        <div className="relative flex flex-col h-full">
          <DrawerHeader>
            <div className="flex items-start justify-between w-full">
              <DrawerTitle>{`Details: ${item?.equipmentName ?? ""}`}</DrawerTitle>
              <DrawerClose asChild>
                <button
                  aria-label="Close"
