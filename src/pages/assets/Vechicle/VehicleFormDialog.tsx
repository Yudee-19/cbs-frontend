  ...
  return (
    <Dialog open={open} onOpenChange={(v) => (!v ? onClose() : null)}>
      <DialogContent className="max-w-2xl max-h-full overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{mode === "add" ? "New Vehicle" : `Edit: ${form.vehicleName ?? ""}`}</DialogTitle>
          <DialogDescription>{mode === "add" ? "Add vehicle details." : "Update vehicle details."}</DialogDescription>
        </DialogHeader>
        ...
        <form
          ...
          onSubmit={async (e) => {
            e.preventDefault();
            // validation based on server rules
            ...
            if (form.year !== undefined && form.year !== null && typeof form.year === "string") {
            ...
            if (form.mileage !== undefined && form.mileage !== null && typeof form.mileage === "string") {
