...
interface Props {
  open: boolean;
  item: FurnitureData | null;
  onClose: () => void;
}

const fmtDate = (iso?: string) => (iso ? new Date(iso).toLocaleDateString() : "-");

export const FurnitureDetailsDrawer: React.FC<Props> = ({ open, item, onClose }) => {
  return (
        ...
        <div className="relative flex flex-col h-full">
          <DrawerHeader>
            <div className="flex items-start justify-between w-full">
              <DrawerTitle>{item ? item.itemName : "Furniture Details"}</DrawerTitle>
              <DrawerClose asChild>
            ...
            {item ? (
              <div className="grid grid-cols-2 gap-4 text-sm">
                {Object.entries({
                  ...
                  Status: item.status,
                  "Purchase Date": fmtDate(item.purchaseDate),
                  Notes: item.notes,
                }).map(([k, v]) => (
                  <div key={k}>
