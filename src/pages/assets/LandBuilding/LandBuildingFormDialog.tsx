...
export const LandBuildingFormDialog: React.FC<Props> = ({ open, mode, form, onChange, onSubmit, onClose, onDelete, submitting = false }) => {
  const footer = (
    <>
      <Button form="property-form" type="submit" size="sm" disabled={submitting}>
  ...
  return (
    <CustomDialog
      ...
      title={mode === "add" ? "New Property" : `Edit: ${form.propertyName ?? ""}`}
      description={mode === "add" ? "Create a new property." : "Update property details."}
      footer={footer}
    >
      <form
        id="property-form"
